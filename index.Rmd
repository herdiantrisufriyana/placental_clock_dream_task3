...
rownames(beta_norm_bmiq) |>
  saveRDS("data/beta_norm_bmiq_rownames.rds")
  setdiff(rownames(beta_norm_bmiq))
```
...
saveRDS(colnames(beta_norm_bmiq), "data/df_train_colnames.rds")
```

```{r Update annotation sample list, include=FALSE}
ano <- ano0[readRDS("data/df_train_colnames.rds"), , drop = FALSE]
...
chorioamnionitis_dmp$Yes_to_No <-
...
```{r GA prediction - Load pre-identified DMPs, eval=FALSE, include=FALSE}
...
```{r GA res-conds pred - Pred set and write it, eval=FALSE, include=FALSE}
for(i in var_ga){
...
```{r GA res-comb pred - Identify datasets cont, eval=FALSE, include=FALSE}
...
ga_450k_est_eval <-
  readRDS("data/ga_450k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]
...
```{r Normal-GA (850k) model evaluation, include=FALSE}
ga_850k_est_eval <-
  readRDS("data/ga_850k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]

ga_850k_est_eval_results <-
  stratified_performance(ga_850k_est_eval, "Normal-GA (850k)") 

ga_850k_est_eval_plot <- stratified_plot_element(ga_850k_est_eval_results)
```
...
ga_resfull_rf_450k_est_eval <-
  readRDS("data/ga_resfull_rf_450k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]
...
```{r ResfullRF-GA (850k) model evaluation, include=FALSE}
ga_resfull_rf_850k_est_eval <-
  readRDS("data/ga_resfull_rf_850k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]

ga_resfull_rf_850k_est_eval_results <-
  stratified_performance(ga_resfull_rf_850k_est_eval, "Resfull-GA* (850k)")

ga_resfull_rf_850k_est_eval_plot <-
  stratified_plot_element(ga_resfull_rf_850k_est_eval_results)
```
...
```{r ResCondsPred-GA (850k) model evaluation, include=FALSE}
ga_res_conds_pred_850k_est_eval <-
  readRDS("data/ga_res_conds_pred_850k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]

ga_res_conds_pred_850k_est_eval_results <-
  stratified_performance(
    ga_res_conds_pred_850k_est_eval
    ,"Res-Conds-GA§ (850k)"
  )

ga_res_conds_pred_850k_est_eval_plot <-
  stratified_plot_element(ga_res_conds_pred_850k_est_eval_results)
```
...
```{r ResComb-GA (850k) model evaluation, include=FALSE}
ga_res_comb_850k_est_eval <-
  readRDS("data/ga_res_comb_850k_est_eval.rds")[
    ano0[readRDS("data/df_train_colnames.rds"), "Sample_accession", drop = TRUE]
    ,
    , drop =FALSE
  ]

ga_res_comb_850k_est_eval_results <-
  stratified_performance(
    ga_res_comb_850k_est_eval
    ,"Res-Comb-GA (850k)"
  )

ga_res_comb_850k_est_eval_plot <-
  stratified_plot_element(ga_res_comb_850k_est_eval_results)
```
...
```{r figure-s7, echo=FALSE, fig.height=3, fig.width=12}
figure_s7
```

```{r Save figure-s7, eval=FALSE, include=FALSE}
ggsave("doc/figure2b.eps", height = 3, width = 12, units = "in")
ggsave("doc/figure2b.svg", height = 3, width = 12, units = "in")
```
...
```{r Save figure-s8, eval=FALSE, fig.height=9, fig.width=12, include=FALSE}
ggsave("doc/figure3b.eps", height = 9, width = 12, units = "in")
ggsave("doc/figure3b.svg", height = 9, width = 12, units = "in")
```

```{r figure-s9, echo=FALSE, fig.height=9, fig.width=12}
stratified_plot(
  ga_res_conds_pred_850k_est_eval_plot, "Res-Conds-GA† (850k)"
  ,ga_res_conds_pred_est_eval_plot, "Res-Conds-GA†"
  ,ga_res_comb_850k_est_eval_plot, "Res-Comb-GA (850k)"
  ,ga_res_comb_est_eval_plot, "Res-Comb-GA"
)
```
...
beta_norm_bmiq |>
  ...
  write_csv("input/Leaderboard_beta_subchallenge3.csv")
```

## Submission 1: GA res-comb-pred elastic net

```{r ResComb-GA prediction - Create model dir, eval=FALSE, include=FALSE}
if (!dir.exists("pcd3_sub1/data")) {
  dir.create("pcd3_sub1/data", recursive = TRUE)
}

if (!dir.exists("pcd3_sub1/inst/extdata")) {
  dir.create("pcd3_sub1/inst/extdata", recursive = TRUE)
}
```

```{r ResComb-GA prediction - Copy the files, eval=FALSE, include=FALSE}
pcd3_sub1_files_non_default <-
  c("data/probe_info_epic.rds"
    ,"data/probe_info_others.rds"
    ,"data/beta_norm_bmiq_rownames.rds"
    ,"data/var_ga.rds"
...
pcd3_sub1_files_non_default |>
  file.copy(paste0("pcd3_sub1/", pcd3_sub1_files_non_default))
```
...
cd pcd3_sub1
docker buildx build --platform linux/amd64 -t docker.synapse.org/syn62407322/ga-res-comb-est-350k:v1.0 --push .
cd ..
```
